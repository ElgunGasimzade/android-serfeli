...
import app.serfeli.model.GuestLoginRequest
...
class SessionManager(context: Context) {
    ...
    fun saveHistory(items: List<app.serfeli.model.RouteHistoryItem>) {
        val json = gson.toJson(items)
        prefs.edit().putString(KEY_HISTORY, json).apply()
    }

    fun getHistory(): List<app.serfeli.model.RouteHistoryItem> {
        val json = prefs.getString(KEY_HISTORY, null) ?: return emptyList()
        val type = object : com.google.gson.reflect.TypeToken<List<app.serfeli.model.RouteHistoryItem>>() {}.type
        return try {
